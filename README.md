...
## 支持的项目
由于工作环境所限，目前仅测试了 **vue2项目**(非ts)、**vue3项目**(非ts)、**Vite项目**(非ts)，其他项目未做测试。理论上是通用的。
...
| distPath           |                          本地打包后生成的目录，如果是Vite项目，需要动态设置`build.outDir`                          | String  |  是   |                          /dist/dev                          |                   /dist/test、/dist/pre、/dist/prod |
| webDir             |                                          服务器部署路径（不可为空或'/'）                                           | String  |  是   |                      /data/website/dev                      |                                                   - |
| script             |                                                      打包命令                                                      | String  |  是   |                          build:dev                          |                   build:test、build:pre、build:prod |
